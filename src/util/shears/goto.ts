...
import type { Parsed, Shear, Context } from "./types";
import { type Query, query } from "./query";
...
declare module "./types" {
  export interface Context<A> {
    ...
    driver?: (
      ...
      Partial<Omit<Context<any>, "data">> & { data: string | Parsed }
    >;
  }
}
...
export interface GoTo {
  <T>(
    ...
    config?: {
      paginate?: {
        selector: string | Shear<Parsed | undefined, string>;
        limit?: number;
  ...
  const execute =
    ...
    async (ctx?: Context<Parsed>) => {
      ...
      if (!isUrl(url)) {
        ...
        if (!result && page.acc > 0 && !options?.paginate?.failOnMissing)
          return [];
        if (!result) {
          const iteration = options?.paginate
            ? ` on paginate: ${page.acc}`
            : "";
          throw new Error(`Did find a result for ${url}${iteration}`);
        }
        if (isUrl(result)) url = result;
        if (result.trim().startsWith("/") && ctx.origin)
          url = `${ctx.origin}${result.trim()}`;
        if (!isUrl(url))
          throw new Error(`Found invalid url ${result} for ${url}`);
      }
...
goto.use = (_ctx) => {
  const _goto: GoTo = (url, selector, options) => (ctx) =>
    goto(url, selector, options)({ ...ctx, ..._ctx } as any);
