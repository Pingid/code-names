import type { CountArrayDepth, Last, Parsed, Shear, Typeof } from "./types";
...
export const query: {
  <T extends ReadonlyArray<string | [string]>>(...args: T): Query<
    CountArrayDepth<T, string | undefined>
  >;
  ...
  async (ctx: any) => {
    ...
    if (is.array()(selector)) {
      ...
      if (!isLast)
        return Promise.all(
          data.map((y) => query(...rest)({ ...ctx, data: y }))
        );
      return data.map((x) => {
    ...
    return Promise.all(
      Object.keys(selector).map((key) =>
        query(selector[key])(ctx).then((value: any) => [key, value])
      )
