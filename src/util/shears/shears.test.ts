...
const html = (
  t: { raw: readonly string[] | ArrayLike<string> },
  ...s: any[]
) => ({
...
it("should select nested query", async () =>
  expect(await query(query("h1"))(html`<div><h1>foo</h1></div>`)).toEqual(
    "foo"
  ));
...
it("should select object query", async () => {
  ...
  expect(await query({ a: "h1", b: "a@href" })(c)).toEqual({
    a: "foo",
    b: "bar",
  });
});
...
it("should select object on all", async () => {
  ...
  expect(await query(["div div"], { a: "p" })(c)).toEqual([
    { a: "foo" },
    { a: "bar" },
  ]);
});
...
it("should handle goto url", async () => {
  ...
  const go = goto.use({
    driver: (page) => Promise.resolve({ data: `<p>${page}</p>` }),
  });
...
it("should handle goto selector", async () => {
  const go = goto.use({
    driver: () => Promise.resolve({ data: `<p>Page 2</p>` }),
  });
...
it("should handle pagination", async () => {
  const go = goto.use({
    driver: (url: string) =>
      Promise.resolve({
        data: `<div><p>${url}</p>><a href="${url}">next</a></div>`,
      }),
  });
  const ctx = html`<div><a href="http://foo.com">bar</a></div>`;
  const result = await query({
    page: go("a@href", query("p"), {
      paginate: { selector: "a@href", limit: 2 },
    }),
  })(ctx);

  expect(result).toEqual({
    page: ["http://foo.com", "http://foo.com", "http://foo.com"],
  });
});
